...
final class DataPageBuilder
{
    ...
    private function buildDataPage(array $rows, FlatColumn $column, ?array $dictionary, ?array $indices) : PageContainer
    {
        ...
        $pageHeader = new PageHeader(Type::DATA_PAGE, \strlen($compressedBuffer), \strlen($pageBuffer), new DataPageHeader(
            (\is_array($dictionary) && \is_array($indices)) ? Encodings::RLE_DICTIONARY : Encodings::PLAIN,
            Encodings::RLE,
            Encodings::RLE,
    ...
    private function buildDataPageV2(array $rows, FlatColumn $column, ?array $dictionary, ?array $indices) : PageContainer
    {
        ...
        $pageHeader = new PageHeader(Type::DATA_PAGE_V2, \strlen($compressedBuffer) + $repetitionsLength + $definitionsLength, \strlen($pageBuffer) + $repetitionsLength + $definitionsLength, null, new DataPageHeaderV2(\count($shredded->definitions), \count(\array_filter($shredded->definitions, function (int $definition) : bool {
            return $definition === 0;
        })), \count($rows), (\is_array($dictionary) && \is_array($indices)) ? Encodings::RLE_DICTIONARY : Encodings::PLAIN, $definitionsLength, $repetitionsLength, !($this->compression === Compressions::UNCOMPRESSED), $statistics), null);
        $pageHeader->toThrift()->write(new TCompactProtocol($pageHeaderBuffer = new TMemoryBuffer()));
