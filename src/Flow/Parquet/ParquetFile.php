...
final class ParquetFile
{
    ...
    /**
     * @var \Flow\Parquet\ParquetFile\Metadata|null
     */
    private $metadata;
     ...
     */
    public function __construct($stream, $byteOrder, DataConverter $dataConverter, Options $options)
    {
        $this->stream = $stream;
    ...
    {
        $reader = new WholeChunkReader(new DataBuilder($this->dataConverter), new PageReader($this->byteOrder, $this->options));

        $yieldedRows = 0;
        $skippedRows = 0;

        /** @var FlowColumnChunk $columnChunk */
        foreach ($this->getColumnChunks($column, $offset) as $columnChunk) {
    ...
    private function read(Column $column, ?int $limit = null, ?int $offset = null) : \Generator
    {
        ...
        if ($column instanceof NestedColumn) {
            ...
            return $this->readStruct($column, false, $limit, $offset);
        }
    ...
    private function readList(NestedColumn $listColumn, ?int $limit = null, ?int $offset = null) : \Generator
    {
        ...
        return $this->readStruct($elementColumn, true, $limit, $offset);
    }
    ...
    private function readMap(NestedColumn $mapColumn, ?int $limit = null, ?int $offset = null) : \Generator
    {
        ...
        if ($valuesColumn->isStruct()) {
            $values = $this->readStruct($valuesColumn, true, $limit, $offset);
        }
    ...
    private function readStruct(NestedColumn $structColumn, bool $isCollection = false, ?int $limit = null, ?int $offset = null) : \Generator
    {
        ...
        foreach ($structColumn->children() as $child) {
            ...
            if ($child instanceof NestedColumn) {
                ...
                $childrenRowsData->attachIterator($this->readStruct($child, $isCollection, $limit, $offset), $child->flatPath());
